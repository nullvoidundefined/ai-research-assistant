...
export default function CollectionManager({ collectionId }: CollectionManagerProps) {
    const queryClient = useQueryClient();
    const [shareUrl, setShareUrl] = useState<string | null>(null);

    const { data, isLoading } = useQuery({
        queryKey: ['collection', collectionId],
        queryFn: () => apiFetch<CollectionDetailData>(`/collections/${collectionId}`),
    });
    ...
    const removeSourceMutation = useMutation({
        mutationFn: (sourceId: string) =>
            apiFetch(`/collections/${collectionId}/sources/${sourceId}`, { method: 'DELETE' }),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['collection', collectionId] }),
    });

    const shareMutation = useMutation({
        mutationFn: () =>
            apiFetch<{ collection: Collection; shareUrl: string }>(`/collections/${collectionId}/share`, {
                method: 'POST',
            }),
        onSuccess: (result) => {
    ...
    const addableSources = allSources.filter((s) => !inCollectionIds.has(s.id) && s.status === 'ready');
    ...
    return (
        <div className={styles.container}>
            <div className={styles.header}>
                ...
                >
                    {shareMutation.isPending ? 'Sharing...' : collection.is_public ? 'Reshare' : 'Share Publicly'}
                </button>
            </div>

            {(shareUrl ?? (collection.share_token && `${window.location.origin}/share/${collection.share_token}`)) && (
                <div className={styles.shareInfo}>
                    ...
                    >
                        {shareUrl ?? `${typeof window !== 'undefined' ? window.location.origin : ''}/share/${collection.share_token}`}
                    </a>
                </div>
            )}
                    ...
                    <ul className={styles.sourceList}>
                        {sources.map((source) => (
                            <li key={source.id} className={styles.sourceItem}>
                                <span className={styles.typeTag}>{source.type.toUpperCase()}</span>
