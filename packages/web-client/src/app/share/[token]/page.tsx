...
async function fetchPublicCollection(token: string): Promise<PublicCollectionData | null> {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? 'http://localhost:3001';
    try {
        const res = await fetch(`${apiUrl}/collections/public/${token}`, { cache: 'no-store' });
                        ...
                        {sources.map((source) => (
                            <li key={source.id} className={styles.sourceItem}>
                                ...
                                <div>
                                    ...
                                    {source.summary && (
                                        <p className={styles.sourceSummary}>{source.summary.slice(0, 200)}</p>
                                    )}
                                </div>
                            </li>
                        ))}
