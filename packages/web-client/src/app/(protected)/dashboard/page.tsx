...
export default function DashboardPage() {
    ...
    const pendingSources = sources.filter((s) => ['pending', 'fetching', 'chunking', 'embedding'].includes(s.status)).length;
            ...
            <div className={styles.section}>
                ...
                {sources.length === 0 ? (
                    <p className={styles.empty}>
                        No sources yet. <Link href="/sources/add">Add your first source</Link>
                    </p>
                ) : (
                    <ul className={styles.recentList}>
                        {sources.slice(0, 5).map((source) => (
                            <li key={source.id} className={styles.recentItem}>
                                <span className={styles.sourceType}>{source.type.toUpperCase()}</span>
