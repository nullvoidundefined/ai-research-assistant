...
import SourceFilters, { type SourceFilterValues } from '@/components/SourceFilters/SourceFilters';
...
export default function SourcesPage() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => apiFetch<{ sources: Source[] }>(`/sources?${queryParams.toString()}`),
    ...
    return (
        <div className={styles.page}>
            <div className={styles.header}>
                <h1>Sources</h1>
                <Link href="/sources/add" className={styles.addBtn}>Add Source</Link>
            </div>
                    ...
                    {sources.map((source) => (
                        <li key={source.id} className={styles.item}>
                            ...
                            <div className={styles.info}>
                                ...
                                {source.summary && (
                                    <p className={styles.summary}>{source.summary.slice(0, 150)}...</p>
                                )}
                            </div>
                            <span
