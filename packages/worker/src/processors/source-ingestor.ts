...
async function extractMetadata(content: string): Promise<{
    ...
    if (!jsonMatch) throw new Error("Failed to extract JSON from metadata response");
...
export async function processSourceIngest(data: {
    ...
    await query("UPDATE sources SET status = 'fetching', error = NULL WHERE id = $1", [sourceId]);
    ...
    try {
        ...
        await query("UPDATE sources SET status = 'failed', error = $1 WHERE id = $2", [
    ...
    await query("UPDATE sources SET status = 'chunking' WHERE id = $1", [sourceId]);

    // Step 3: Extract metadata
    let metadata: { title: string | null; author: string | null; publishedDate: string | null; summary: string };
    try {
        metadata = await extractMetadata(content);
        await query(
            "UPDATE sources SET title = $1, author = $2, published_date = $3, summary = $4 WHERE id = $5",
            [metadata.title, metadata.author, metadata.publishedDate, metadata.summary, sourceId]
        );
    } catch (err) {
    ...
    await query("UPDATE sources SET status = 'embedding' WHERE id = $1", [sourceId]);
    ...
    for (let i = 0; i < chunks.length; i++) {
        ...
        await query(
            ...
            [sourceId, userId, i, chunks[i], Math.ceil(chunks[i].length / 4), embeddingStr]
        );
    }
