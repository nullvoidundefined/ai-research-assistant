import type { SourceStatus, SourceType } from '@research/common';
...
export async function updateSource(
  ...
  if (data.publishedDate !== undefined) {
    ...
    fields.push(`published_date = $${params.length}`);
  }
  ...
  if (data.totalChunks !== undefined) {
    params.push(data.totalChunks);
    fields.push(`total_chunks = $${params.length}`);
  }
...
export async function updateSourceStatus(
  ...
  await query('UPDATE sources SET status = $1, error = $2 WHERE id = $3', [
    status,
    error ?? null,
    id,
  ]);
