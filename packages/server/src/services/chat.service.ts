...
import { getConversationMessages, saveMessage } from "app/repositories/messages/messages.js";
import { buildQASystemPrompt, buildContextBlock } from "app/prompts/qa-system.js";
...
export async function handleChatStream(
    ...
    try {
        ...
        let historyContent: Array<{ role: "user" | "assistant"; content: string }> = history.map(
        ...
        const userMessageWithContext = chunks.length > 0
        ...
        // Save messages
        const userMsg = await saveMessage(conversationId, "user", message, [], Math.ceil(message.length / 4));
